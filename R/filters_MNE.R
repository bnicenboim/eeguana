...
create_filter <- function(data, sampling_rate=NULL, l_freq=NULL, h_freq=NULL, filter_length="auto",
    ...
    ## if method == "fir":
    construct_fir_filter(sampling_rate, freq, gain, filter_length, phase,
...
next_fast_len <- function(target){
    ...
    ## """ 
    hams = c(8, 9, 10, 12, 15, 16, 18, 20, 24, 25, 27, 30, 32, 36, 40, 45, 48,
...
overlap_add_filter <- function(x, h, n_fft=NULL, phase="zero",
    ...
    if (is.null(n_fft)){ 
        ...
        if (max_fft >= min_fft){
            ...
            N <- 2 ^ seq.int(ceiling(log2(min_fft)),
