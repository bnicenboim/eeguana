...
read_dat <- function(file, common_info = NULL, chan_info = NULL, events = NULL,
  ...
  data <- purrr::pmap_dfr(list(beg_segs, s0, end_segs),
    .id = ".id",
  ...
  eegble <- new_eegbl(
    signal = data, events = seg_events, channels = chan_info,
    info = eeg_info, segments = segments
  )
...
segment_events <- function(events, beg_segs, s0, end_segs) {
  purrr::pmap_dfr(list(beg_segs, s0, end_segs),
    ...
    function(b, s0, e) events %>%
        ...
        dplyr::filter(
          sample >= b | sample + size - 1 >= b,
          # start before the end
          sample <= e
        ) %>%
        dplyr::mutate(
          size = dplyr::if_else(sample < b, b - size, size),
          sample = dplyr::case_when(
            sample >= b ~ sample - s0 + 1L,
            sample < b ~ b - s0 + 1L
          )
        )
  ) %>%
...
read_vmrk <- function(file) {
  ...
  col_names <- c(
    "Mk_number=Type", "description", "sample",
    "size", "channel", "date"
  )
