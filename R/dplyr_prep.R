...
# Based on https://github.com/markfairbanks/tidytable/blob/46f95407ba16abb414a44dbf8279a4a63fdb6a01/R/utils-prep_exprs.R
...
prep_expr_call <- function(x, data, .by = NULL, j = FALSE) {
  if (is_call(x, c( "n"))) {
    quote(.N)
  } else if (is_call(x, c( "desc"))) {
    x[[1]] <- sym("-")
    x[[2]] <- get_expr(x[[2]])
    x
  } else if (is_call(x, c("row_number",  "cur_group_rows"))) {
    quote(1:.N)
  } else if (is_call(x, c( "cur_group_id"))) {
    quote(.GRP)
