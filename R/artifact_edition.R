...
#' @return An eeg_lst.
#' @export
eeg_events_to_NA <- function(x, ..., all_chs = FALSE, entire_seg = TRUE,
                                     drop_events = TRUE) {
  UseMethod("eeg_events_to_NA")
}

#' @export
eeg_events_to_NA.eeg_lst <- function(x, ..., all_chs = FALSE, entire_seg = TRUE,
                                     drop_events = TRUE) {
  dots <- rlang::enquos(...)
  ...
  if (all_chs) baddies <- dplyr::mutate(baddies, .channel = NA)
