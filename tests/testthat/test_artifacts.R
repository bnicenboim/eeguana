...
data <- eeg_lst(
    signal_tbl = signal, 
    segments =  tibble(.id = 1L, .recording = "recording1", segment =  1L)
)
...
data_more <- eeg_lst(
    ...
    ,
    segments =  tibble(.id = seq.int(4), .recording = paste0("recording",c(1,1,2,2)), segment =  seq.int(4))
)
...
test_that("window of 22 elements with different .id", {
    ...
    art_events <- data_more %>%
        eeg_artif_step(threshold = .01,window=2/500, lim=c(-10/500,10/500), unit = "second") %>% events_tbl()
    ...
    art_events <- data_more %>%
        eeg_artif_step(Fz, threshold = .01,window=2/500, lim=c(-10/500,10/500), unit = "second") %>% events_tbl()
    expect_equal(art_events[.channel=="Fz",]$.initial %>% as.numeric(), c(pmax(Fzim-11,1)) )
    expect_equal(art_events[.channel=="Fz",]$.final %>% as.numeric(),c(pmin(Fzfm+10,250)) )
