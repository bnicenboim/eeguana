...
test_that("can clean files with entire_seg = FALSE", {
  ...
  expect_equal(all(is.na(clean_data$signal_tbl[clean_data$signal_tbl$.sample_id %in% seq(-2, -3 + 3 - 1) &
    clean_data$signal_tbl$.id == 1, c("X", "Y")])), TRUE)
  expect_equal(all(is.na(clean_data$signal_tbl[clean_data$signal_tbl$.sample_id %in% seq(2, 2 + 2 - 1) &
    clean_data$signal_tbl$.id == 1, c("X")])), TRUE)
  expect_equal(all(is.na(clean_data$signal_tbl[clean_data$signal_tbl$.sample_id %in% seq(2, 2 + 2 - 1) &
    clean_data$signal_tbl$.id == 1, c("Y")])), FALSE)
  expect_equal(all(is.na(clean_data$signal_tbl[clean_data$signal_tbl$.sample_id %in% seq(2, 2 + 1 - 1) &
    clean_data$signal_tbl$.id == 2, c("Y")])), TRUE)
  expect_equal(all(is.na(clean_data$signal_tbl[clean_data$signal_tbl$.sample_id %in% seq(2, 2 + 1 - 1) &
    clean_data$signal_tbl$.id == 2, c("X")])), FALSE)
  expect_equal(all(is.na(clean_data$signal_tbl[clean_data$signal_tbl$.id == 1, c("X", "Y")])), FALSE)
  expect_equal(all(is.na(clean_data$signal_tbl[clean_data$signal_tbl$.id == 2, c("Y")])), FALSE)
  expect_equal(all(is.na(clean_data$signal_tbl[clean_data$signal_tbl$.id == 2, c("X")])), FALSE)
})
...
test_that("can clean whole channels in files", {
  ...
  expect_equal(all(is.na(clean_data_chan$signal_tbl[clean_data_chan$signal_tbl$.sample_id %in% seq(-2, -3 + 3 - 1) &
    clean_data_chan$signal_tbl$.id == 1, c("X", "Y")])), TRUE)
  expect_equal(all(is.na(clean_data_chan$signal_tbl[clean_data_chan$signal_tbl$.sample_id %in% seq(2, 2 + 2 - 1) &
    clean_data_chan$signal_tbl$.id == 1, c("X")])), TRUE)
  expect_equal(all(is.na(clean_data_chan$signal_tbl[clean_data_chan$signal_tbl$.sample_id %in% seq(2, 2 + 2 - 1) &
    clean_data_chan$signal_tbl$.id == 1, c("Y")])), TRUE)
  expect_equal(all(is.na(clean_data_chan$signal_tbl[clean_data_chan$signal_tbl$.sample_id %in% seq(2, 2 + 1 - 1) &
    clean_data_chan$signal_tbl$.id == 2, c("Y")])), TRUE)
  expect_equal(all(is.na(clean_data_chan$signal_tbl[clean_data_chan$signal_tbl$.sample_id %in% seq(2, 2 + 1 - 1) &
    clean_data_chan$signal_tbl$.id == 2, c("X")])), TRUE)
  expect_equal(all(is.na(clean_data_chan$signal_tbl[clean_data_chan$signal_tbl$.id == 1, c("X", "Y")])), FALSE)
  expect_equal(all(is.na(clean_data_chan$signal_tbl[clean_data_chan$signal_tbl$.id == 2, c("Y")])), FALSE)
  expect_equal(all(is.na(clean_data_chan$signal_tbl[clean_data_chan$signal_tbl$.id == 2, c("X")])), FALSE)
})
...
test_that("can clean whole segments in files", {
  ...
  expect_equal(all(is.na(clean_data_seg$signal_tbl[clean_data_seg$signal_tbl$.id == 1, c("X", "Y")])), TRUE)
  expect_equal(all(is.na(clean_data_seg$signal_tbl[clean_data_seg$signal_tbl$.id == 2, c("Y")])), TRUE)
  expect_equal(all(is.na(clean_data_seg$signal_tbl[clean_data_seg$signal_tbl$.id == 2, c("X")])), FALSE)
})
...
test_that("can segment", {
  ...
  expect_equal(data$signal_tbl, data_s$signal_tbl)
  ...
  expect_equal(d$signal_tbl, d_rec$signal_tbl)
  ...
  expect_equal(nrow(d_0$signal_tbl), 10)
  expect_equal(d_0$signal_tbl, d_0_0$signal_tbl)
  ...
  expect_equal(s1$signal_tbl$X[1], data0$signal_tbl$X[6])
  expect_equal(nrow(s1$signal_tbl), 4)
  ...
  expect_equal(nrow(double$signal_tbl), 40)
  d00 <- segment(data0, type == "Time 0", lim = c(0, 1), unit = "sample")
  expect_equal(all(d00$events$.sample_0 + d00$events$.size - 1 <= max(d00$signal_tbl$.sample_id)), TRUE)
  expect_equal(all(s1$events$.sample_0 + s1$events$.size - 1 <= max(s1$signal_tbl$.sample_id)), TRUE)
  expect_equal(all(s1_u$events$.sample_0 + s1_u$events$.size - 1 <= max(s1_u$signal_tbl$.sample_id)), TRUE)
  expect_equal(all(s1_u2$events$.sample_0 + s1_u2$events$.size - 1 <= max(s1_u2$signal_tbl$.sample_id)), TRUE)
  expect_equal(all(s1_u2$events$.sample_0 + s1_u2$events$.size - 1 <= max(s1_u2$signal_tbl$.sample_id)), TRUE)
})


baselines <- dplyr::summarize(dplyr::group_by(
  dplyr::filter(eeguana:::declass(data$signal_tbl)$tbl, .sample_id <= 0),
  .id
), bX = mean(X), bY = mean(Y))
signal_with_baselines <- dplyr::left_join(eeguana:::declass(data$signal_tbl)$tbl, baselines)
...
test_that("baseline works", {
  expect_equal(eeguana:::declass(baselined$signal_tbl)$tbl$X, signal_with_baselines$new_X)
  expect_equal(eeguana:::declass(baselined$signal_tbl)$tbl$Y, signal_with_baselines$new_Y)
})
