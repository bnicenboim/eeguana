context("test at and if dplyr verbs")
...
test_that("summarize ats (and rename) no extra args", {
  ...
  signal_means <- eeguana:::left_join_dt(data_grouped_descr$.signal,
...
data_grouped_descr %>% dplyr::filter(.id !=2) %>% dplyr::group_by( .sample) %>%
