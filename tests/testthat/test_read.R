...
#   expect_equal(max(eeg_lst_ascii$signal_tbl$.sample_id), 1000)
#   expect_equal(max(eeg_lst_ascii$signal_tbl$.id), max(eeg_lst_ascii$segments$.id))
...
test_that("can read binary dat files", {
  expect_equal(max(eeg_lst_av$signal_tbl$.sample_id), 500)
