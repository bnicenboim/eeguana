...
test_that("low pass signal",{
    data_sin_X1 <- data_sin_X1 %>% filter(as_time(.sample) %>% between(.25,1.75))
    data_sin <- data_sin %>% filter(as_time(.sample) %>% between(.25,1.75))
...
test_that("high pass signal",{
    data_sin_X3 <- data_sin_X3 %>% filter(as_time(.sample) %>% between(.25,1.75))
    data_sin <- data_sin %>% filter(as_time(.sample) %>% between(.25,1.75))
...
test_that("band pass signal",{
    data_sin_X2 <- data_sin_X2 %>% filter(as_time(.sample) %>% between(.25,1.75))
    data_sin <- data_sin %>% filter(as_time(.sample) %>% between(.25,1.75))
...
test_that("band stop signal",{
    data_sin_X1X3 <- data_sin_X1X3 %>% filter(as_time(.sample) %>% between(.25,1.75))
    data_sin <- data_sin %>% filter(as_time(.sample) %>% between(.25,1.75))
...
test_that("low pass signal",{
    data_sin_more_X1 <- data_sin_more_X1 %>% filter(as_time(.sample) %>% between(.15,.4))
    data_sin_more <- data_sin_more %>% filter(as_time(.sample) %>% between(.15,.4))
...
test_that("high pass signal",{
    data_sin_more_X3 <- data_sin_more_X3 %>% filter(as_time(.sample) %>% between(.15,.4))
    data_sin_more <- data_sin_more %>% filter(as_time(.sample) %>% between(.15,.4))
...
test_that("band pass signal",{
    data_sin_more_X2 <- data_sin_more_X2 %>% filter(as_time(.sample) %>% between(.15,.4))
    data_sin_more <- data_sin_more %>% filter(as_time(.sample) %>% between(.15,.4))
...
test_that("band stop signal",{
    data_sin_more_X1X3 <- data_sin_more_X1X3 %>% filter(as_time(.sample) %>% between(.15,.4))
    data_sin_more <- data_sin_more %>% filter(as_time(.sample) %>% between(.15,.4))
...
test_that("band pass signal, sel",{
    data_sin_more_X2_onlyX1X2 <- data_sin_more_X2_onlyX1X2 %>%
        filter(as_time(.sample) %>% between(.15,.4))
    data_sin_more_X2 <- data_sin_more_X2 %>% filter(as_time(.sample) %>% between(.15,.4))
    data_sin_more <- data_sin_more %>% filter(as_time(.sample) %>% between(.15,.4))
