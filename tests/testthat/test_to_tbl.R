...
cond1 <- eeg_lst(
  ...
  segments =  tibble(.id = 1L, .recording = "recording1", segment = 1, .type = "initial")
)


cond2 <- eeg_lst(
  signal_tbl =
  tibble(X = sin(1:10) + .1, Y = cos(1:10) + .1,
    .id = rep(1L, 10),
    .sample = sample_int(seq(-4L, 5L), sampling_rate = 500)),
   channels_tbl =  tibble(
      .channel = c("X", "Y"), .reference = NA, theta = NA, phi = NA,
  ...
  segments =  tibble(.id = 1L, .recording = "recording2", segment = 1, .type = "initial")
)
...
cond3 <- eeg_lst(
 ...
 ,
  ...
  segments =  tibble(.id = 1L, .recording = "recording3", segment = 1, .type = "initial")
)
