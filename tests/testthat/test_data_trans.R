...
cond1 <- new_eegbl(signal = dplyr::tibble(.id= rep(1L,10), sample= seq(-4L,5L), X = sin(1:10), Y = cos(1:10)),
...
cond2 <- eegble(signal = dplyr::tibble(.id= rep(1L,10), sample= seq(-4L,5L), X = sin(1:10) + .1, Y = cos(1:10) + .1),
           events = dplyr::tribble(~.id, ~type,        ~description, ~sample, ~size, ~channel,
                              1L, "New Segment",  NA,          -4L,       1L,   NA,
                              1L, "Time 0",       NA,           1L,       1L,   NA),
           channels = dplyr::tibble(labels = c("X", "Y"), theta = NA, phi = NA, radius = NA, x = NA, y = NA, z = NA),
...
cond3 <- eegble(signal = dplyr::tibble(.id= rep(1L,10), sample= seq(-4L,5L), X = sin(1:10) + .1, Y = cos(1:10) + .1),
           events = dplyr::tribble(~.id, ~type,        ~description, ~sample, ~size, ~channel,
