...
N4_b_df <- as.data.frame(b, chans = N400_c)
...
test_that("can transform to data frame", {
  expect_equal(nrow(N4_b_df), 1207500)
  expect_equal(ncol(N4_b_df), 6)
  expect_equal(colnames(N4_b_df), c("id","segment","sample","time", "channel","amplitude"))
})
