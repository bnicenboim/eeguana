...
## Real dplyr vs eeguana dplyr-like functions

The package {eeguana} allows for functions that *look* like [dplyr] functions but they trully are powered by [data.table]. This means that there are subtle differences between dplyr's behavior and eeguana dplyr-like function behavior.
...
Grouped mutations behave slightly different than ungrouped ones.

Channel properties are removed if the data is ungrouped and one overwrites a channel.
...
## Refering back to recently created channels
...
```{r}
new_data <- data_faces_ERPs %>%
  mutate(VEOG = scale(VEOG), Y = VEOG * 10)
...
For now `across()` and  `where()` can be replaced by `_at` and `_if` version of the dplur verbs
