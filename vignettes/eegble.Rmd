...
`eegble` has wrappers for some `dplyr` commands for the EEG data. Since the `eegble` objects contain different data frames, first one needs to decide on which part of the object the `dplyr` command will `act_on`. These commands always return the entire `eegble` object so that they can be piped using `magrittr`'s pipe, `%>%`.
...
of the occipital and parietal lobes using the capabilities inherited from `dplyr`.
...
```{r, fig.dim = c(10,15), out.width = "100%"}
faces_segs_some %>%  act_on(signal) %>% 
                transmute(Occipital = (O1 + O2 + Oz)/3,
                          Parietal = (P3+ P4 + P7 + P8 + Pz)/3 ) %>% 
                act_on(segments) %>%
                mutate(condition =
                 if_else(description == "s70", "faces", "non-faces")) %>%
                plot_gg() + 
