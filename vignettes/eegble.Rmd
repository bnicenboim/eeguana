...
repetition and generates an object of 786 MB. While this is still ok here, a long format quickly becomes prohibitive in real settings with longer recordings and with
several subjects.
...
of the occipital and parietal lobes using the capabilities inherited from `dplyr`, and the special function, `channelMeans`, a wrapper for `rowMeans`, which takes as arguments the eegble object, the channels, and whether missing values should be omitted from the calculations.

<!-- `dplyr` together with `pmap_dbl` from `purrr`. (Here `pmap_dbl` is taking three column and doing the rowwise mean of them taking into account NAs, the first example `pmap_dbl(list(O1,  O2,  Oz), mean , na.rm = TRUE)` would be equivalent to `(O1 + O2 + Oz)/3` if we were sure there are no `NA`s). -->

<!--
Occipital = pmap_dbl(list(O1,  O2,  Oz), mean, na.rm = TRUE), 
                          Parietal = pmap_dbl(list(P3, P4, P7, P8, Pz), 
                                                              mean, na.rm = TRUE)
                                                              -->
                                                              
```{r, fig.dim = c(10,15), out.width = "100%"}
faces_segs_some %>%  
                transmute(Occipital = mean_by_sample_ch(O1, O2, Oz, na.rm = TRUE),
                          Parietal = mean_by_sample_ch(P3, P4, P7,  P8, Pz, na.rm = TRUE)) %>% 
                plot_gg() + 
                geom_line(alpha = .1, aes(group = .id, color = condition)) +
...
However, this can be also done in the without transforming the `eegble` object:
