...
```{r,   fig.dim = c(5,5),fig.show='hold',   out.width = "45%"}
#order
data_summary <- data_judg_s_p %>% group_by(segment) %>% summarize_all_ch(var)
...
data_summary %>%  summarize_all_ch(mean) %>% 
                   plot_gg(x = amplitude, y= channel) + 
                   geom_point() 

data_summary %>%  chs_mean() %>% 
