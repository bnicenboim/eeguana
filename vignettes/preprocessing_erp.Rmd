...
However, we want the ERP based on the trigger "S141" that precedes any of these two triggers. We edit the events table (using dplyr functions: `mutate`, `case_when`, `lead`) to indicate to which condition each trigger "S141" belongs, and then we can segment based on these conditions using *eeguana*'s `segment()` (We are going to create a segment slightly bigger than what we want (\[-.2,1\] seconds) in order to discard the artifacts at edges that will be created by the filtering):
...
We can have a look at one of the trials (the second one) of one channel (channel 27) using `plot()` to see the action of the filter.

```{r , fig.dim = c(10,7)}
X27_filtered <- data_judg_s_p %>% filter(segment == 2) %>% 
                  select(X27) %>% 
                  mutate(filter = "Filtered")

X27_unfiltered <- data_judg_s %>% filter(segment == 2) %>% 
                  select(X27) %>% 
...
Now we filter the samples that might be distorted by the filter.

```{r , fig.dim = c(10,10)}
data_judg_s_p <- data_judg_s_p %>% filter(as_time(.sample_id) %>% between(-.2,1)) 
...
#ggplotly(plot_general)
#ggplotly(plot_channels)
#ggplotly(plot_segments)

```

```{r, fig.dim = c(5,5),fig.show='hold',echo= FALSE, out.width = "45%"}
ggplotly(plot_general)
ggplotly(plot_channels)
ggplotly(plot_segments)
```
