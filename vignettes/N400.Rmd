...
The EEG dataset used in this script is available in [](). In the experiment, subjects made
...
```{r}
plot(s1_N400) +annotate_events(events_tbl(s1_N400))
...
```{r}
s1_N400_filt <- eeg_filt_band_pass(s1_N400,freq = c(.1,30))
...
df <- try_to_downsample(s1_N400_s_ica, 2000)
df <- df %>%  data.table::as.data.table()

dt <- data.table::copy(df$signal)
dt[, intersect(channel_names(df), df$signal):= NULL ]
components <- df$signal %>% dplyr::select_at(vars(one_of(channel_names(df)))) %>%
    data.table::melt(variable.name = ".source",
                     measure.vars = component_names(df),
                     value.name = ".value")
components[,.type := "component"][
   ,.source := as.character(.source)]

     channels <- df$signal %>% dplyr::select_at(-one_of(component_names(df)))) %>%
          data.table::melt(variable.name = ".source",
                           measure.vars = intersect(colnames(df$signal), channel_names(df)),
                           value.name = ".value")
...
annotate_events(events_tbl(crazy))

pp <- ggplot(crazy, aes(x=time, y = .value, group=.id)) +geom_line()
...
eeg_artif_minmax(-HEOG, -VEOG, threshold =100, window=150, unit = "ms") %>%
    ...
    eeg_artif_step(VEOG, HEOG, threshold = 50, window=200, unit = "ms")

events_tbl(s1_N400_s) %>% count(type,description)

 
#####
sH <- filter(s1_N400_s, .id %in% filter(events_tbl(s1_N400_s), .channel == "HEOG")$.id )

filter(s1_N400_s, .id %in% c(1,2,3) )

## profvis({ fs
##     data_N400_s %>% eeg_artif_minmax(-HEOG, -VEOG, difference =100, window=150, unit = "ms")
## })
## piece <- filter(s1_N400_s, .id %in% 1:30)
piece <- filter(sH, .id %in% c(1,10,13,20,25,32)) %>% select(HEOG,Fp1)
plot(piece) + facet_grid(.source~.id) + annotate_events(events_tbl(piece))

#####



s1_N400_s_clean <- s1_N400_s %>% eeg_events_to_NA(type == "artifact" & !.channel %in% c("HEOG","VEOG"), all_chans = TRUE,entire_seg = FALSE)

s1_N400_s_clean_ica <- eeg_ica(s1_N400_s_clean, -HEOG, -VEOG, -M1, -M2, na.rm=TRUE)
