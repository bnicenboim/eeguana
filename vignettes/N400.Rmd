...
s1_N400_ls <- eeg_segment(s1_N400_filt, description == "s130", end= description == "s134") %>%
    eeg_artif_amplitude(threshold = c(-200,200),events_lim = c(-1000,1000), unit = "ms")

ids <- filter(events_tbl(s1_N400_ls), type == "artifact") %>%
    pull(.id) %>%
    unique

plot_artif <- filter(s1_N400_ls, .id %in% ids[1:10]) %>%
     plot(max_sample =2000) + facet_grid(.~.id)
...
filter(complete.cases(signal_tbl(s1_N400_ica)))
