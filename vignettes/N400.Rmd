...
events_tbl(s1_N400_filt) %>%
    filter(description == "s130") %>% pull(.sample_0) %>% diff
                                        ...
                                        #2200 should be enough

s1_N400_s <-
    s1_N400_filt %>% select(-type,-description) %>%
    eeg_segment(description == "s130",lim=  c(0,2200), unit = "sample")

s1_N400_s <- s1_N400_s %>% 
    eeg_artif_minmax(-HEOG, -VEOG, difference =100, window=150, unit = "ms") %>%
    eeg_artif_step(-HEOG, -VEOG,difference = 50)
...
s1_N400_s_clean_ica <- eeg_ica(s1_N400_s_clean, -HEOG, -VEOG)
