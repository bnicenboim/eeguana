...
events_tbl(s1_N400_filt) %>%
    filter(description == "s130") %>% pull(.sample_0) %>% diff
...
s1_N400_s <- s1_N400_s %>% 
    ...
    eeg_artif_minmax(HEOG, VEOG, threshold =30, window=150, unit = "ms") %>%
    eeg_artif_step(HEOG, VEOG,threshold = 30, window=200, unit = "ms")
...
s1_N400_s_clean <- s1_N400_s %>% eeg_events_to_NA(type == "artifact",all_chans = TRUE,entire_seg = FALSE)

s1_N400_s_clean_ica <- eeg_ica(s1_N400_s_clean, -HEOG, -VEOG, -M1, -M2, na.rm=TRUE)

s1_N400_s_clean$signal %>% select_if(is_channel_dbl) %>% as.matrix() %>% abs() %>% mean(na.rm=TRUE)
piece_reconstr <- as_eeg_lst(s1_N400_s_clean_ica) %>% filter(.id %in% 1:5)

piece_reconstr <- s1_N400_s_clean %>% filter(.id %in% 1:5) %>% 
```
